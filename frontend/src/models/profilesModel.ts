...
export class ProfilesModel {
    ...
    async getFollowees() {
        ...
        const response = await fetch('dev-api/followed', RequestOptions.getRequestOptions());
    ...
    async getMostPopularProfiles(quantity: number = 20) : Promise<SocialProfile[]> {
        ...
        const response = await fetch(`dev-api/profiles/popular/${quantity}`, RequestOptions.getRequestOptions());
