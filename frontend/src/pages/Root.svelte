<script lang="ts">
    ...
    completed.then(logged => {
        if(logged) {
            ...
            window.location.href = `https://bunnyfood-dev.auth.eu-central-1.amazoncognito.com/login?client_id=2k5d4g58072evbdqloqkuksd5u&response_type=token&redirect_uri=${window.location.protocol}//${window.location.host}`;
        }
    });
</script>
